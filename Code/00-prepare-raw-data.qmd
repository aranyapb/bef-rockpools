---
title: "00-prepare-raw-data"
---

## Creating 6 raw datasets to use in data analysis - 

Alpha scale (individual pools within an inselberg) data for all taxa, only active dispersers and only passive dispersers respectively- 
(1) alpha_all_raw, (2) alpha_active_raw, (3) alpha_passive_raw.
Includes the following columns (for each pool) - Inselberg, Pool, Alpha richness, Alpha standard error (ASE), Gamma diversity, Biomass, Depth, Gamma, Gamma standard error (GSE), Climate PC1 and Climate PC2.

Gamma scale (between inselbergs) data for all taxa, only active dispersers and only passive dispersers respectively - 
(4) gamma_all_raw, (5) gamma_active_raw, (6) gamma_passive raw
Includes the following columns (for each inselberg) - Inselberg, Pool, Gamma diversity, Median biomass, Median Depth, Mean log biomass, Mean log depth, Gamma, Gamma standard error (GSE),Prominence, Climate PC1 and Climate PC2.
...
BM_df <- read_csv(here::here("Data-raw", "pool-community-biomass-data.csv"))
...
#-------------------------------------------------------------------------------
## Sepparating datasets for only active and passive dispersers

# Load data
AP_df <- read_csv(here::here("Data-raw", "active-passive-split-taxa.csv"))
...
ab_df <- read_csv(here::here("Data-raw", "pool-community-abundance-data.csv"))
...
Raw_df <- read_csv(here::here("Data-raw","raw-data-alpha.csv"))
...
```{r}
Raw_data <- read_csv(here::here("Data-raw","raw-data-gamma.csv"))

prom_df <- Raw_data %>% dplyr::select(Inselberg, Prominence)
```

# 6. BIOCLIM variables (CLIMATE PC1 and PC2) 
...
Raw_df_1 <- read_csv(here::here("Data-raw","raw-data-gamma.csv"))
...
bio <- worldclim_global(var="bio", res=2.5,path = here::here("Data-raw"), lon=coords$Longitude, lat=coords$Latitude) 

# Load the downloaded raster files 
raster_files <- list.files(here::here("Data-raw", "climate", "wc2.1_2.5m"), pattern = "*.tif", full.names = TRUE)
...
readr::write_csv(x = bioclim, file = here::here("Data-raw", "bioclim-data.csv"))
...
# write to a .csv file for use in later analyses
readr::write_csv(x = pca2_scores, file = here::here("Data-raw", "bioclim-pca-scores.csv"))
...
readr::write_csv(alpha_all_raw, here::here("Data-raw","alpha-all-raw.csv"))
...
readr::write_csv(alpha_active_raw, here::here("Data-raw","alpha-active-raw.csv"))
...
readr::write_csv(alpha_passive_raw, here::here("Data-raw","alpha-passive-raw.csv"))
...
readr::write_csv(gamma_all_raw, here::here("Data-raw","gamma-all-raw.csv"))
...
readr::write_csv(gamma_active_raw, here::here("Data-raw","gamma-active-raw.csv"))
...
readr::write_csv(gamma_passive_raw, here::here("Data-raw","gamma-passive-raw.csv"))
