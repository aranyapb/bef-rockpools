---
title: "Passive dispersers: Hypothesis 1"
format: 
  html:
...
This shows that the model explains only a small proportion of the variation in log-transformed biomass through the fixed effects alone. Including random effects improves explanatory power somewhat, but overall, both fixed and random factors together still explain only a modest share of the variation in biomass. Less than for thw model with all taxa considered. 
...
These likelihood ratio tests showed that only the log-transformed pool depth and climate PC2 significantly affected the log-transformed biomass. Therefore, there was no effect of log-transformed gamma diversity on log-transformed biomass.
...
saveRDS(object = p1[[1]], file = here::here(paste0("Figures/fig_5", p1[[2]], ".rds")))
```
