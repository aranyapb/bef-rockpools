...
When modeling the effect of log-transformed alpha diversity on log-transformed pool biomass non-linearly using a basis spline, the linear random slope offset (which allows the effect to vary across inselbergs) did not fit the data significantly better, and thus this doesn't support the hypothesis. 
