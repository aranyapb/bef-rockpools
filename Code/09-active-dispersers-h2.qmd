---
title: "Active dispersers: Hypothesis 2"
format: 
  html:
...
```
The fixed effects (logalpha and logdepth) explain some of the variation in log biomass, while the inclusion of random effects (accounting for variability across inselbergs) increases the explanatory power.
...
Based on this model-fitting procedure, we were unable to find models that were able to pass all the model specification tests from the DHARMa package.
...
h2_dat_zeros <- h2_dat[h2_dat$logbiomass != min(h2_dat$logbiomass), ]

# fit and compare the models
model2_comparison <- 
  fit_and_compare_lmm_models(formulas = model2_list, 
                             data = h2_dat_zeros, n_sim = 1000)
...
However, this also did not fully solve the issues with the model residual patterns.
...
# remove the zeros
summary(dat)
hist(log(dat$biomass[dat$biomass < 50 & dat$biomass > 0]), breaks = 20)
```
...
sum(dat$biomass < 50 & dat$biomass > 0)/nrow(dat)
```
...
h2_dat_sub <-
  dat |>
  dplyr::filter(biomass < 50, biomass > 0) |>
...
active_beta <- dplyr::tibble(inselberg = row.names(lm2_re),
...
# export this dataset
saveRDS(active_beta,
        file = here::here(file.path("Data", "active-dispersers-beta-h3-data.rds")))
```
