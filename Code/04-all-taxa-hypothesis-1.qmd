...
These likelihood ratio tests showed that only the log-transformed pool depth significantly affected the log-transformed. Therefore, there was no effect of log-transformed gamma diversity on log-transforemd biomass.

#### Data visualisation

We plotted the model predictions for the effect log-transformed gamma diversity on log-transformed pool biomass while holding PC1, PC2 and log-transformed pool depth at their mean values:
