...
Modelling log-transformed gamma as a non-linear basis-spline function substantially improves the patterns in the residuals.

#### Model (with basis spline)

```{r}
# fit the model with a basis spline for gamma
lm1_bs <- lmer(logbiomass ~ bs(loggamma, degree = 3) + logdepth + pc1 + pc2 + (1 | inselberg),
               data = h1_dat, REML = FALSE)
```
...
```{r}
# plot the model fit
plot(predict(lm1_bs), h1_dat$logbiomass, 
     xlab = "Predicted E(log(biomass)) (mg)", ylab = "Observed log(biomass) (mg)")
abline(0, 1)
```
...
```{r}
# hypothesis test
drop1(lm1_bs, test = "Chisq")
```

These results were therefore robust to including a non-linear relationship between log-transformed gamma diversity and log-transformed biomass in the model.
